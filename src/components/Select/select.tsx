...
export const Select:FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames('grace-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="grace-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
            ...
            timeout={300}
          >
          <ul className="grace-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="grace-selected-tags" style={{maxWidth: containerWidth.current - 32}}> 
          {
            selectedValues.map((value, index) => {
              return (
                <span className="grace-tag" key={`tag-${index}`}>
                  {value}
